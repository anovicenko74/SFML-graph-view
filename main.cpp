...
int main()
{
    ...
    public:
        ...
        void print() const {
            ...
            // Загружаем шрифт
            sf::Font font;
            if (!font.loadFromFile("fonts/tuffy.ttf")) {
                return;
            }
            ...
            while (window.isOpen()) {
                ...
                for (int i = 0; i < numVertices; ++i) {
                    ...
                    text.setFillColor(sf::Color::Blue); // Цвет текста
                    text.setPosition(
                        positions[i].x - text.getLocalBounds().width / 2 - 15,
                        positions[i].y - text.getLocalBounds().height / 2 - 15
                    );
